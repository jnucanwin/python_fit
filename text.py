...
import os
time = get_time_difference(10)#和起始图像的时间差，单位秒
i = 0
...
for v in range(-5,15):
    ...
    for fit_name in filename[start:end]:
        if "fit" in fit_name or "fits" in fit_name:
            ...
            img += 1
    print(str(v)+"\n\n\n")
    fusion_fit("E:\\MyPro\\python_fit2\\fit_image\\result\\asteroid\\" +str(start)+"_"+str(end) +"_"+ str( round(time[0]/6000*v,3))+"_mean_fusion.fit", data_sum, 1)
f.close()
